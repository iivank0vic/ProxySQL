...
function ReplaceConf { 
    ...
    sed -i "s/pr0xyPass/$PROXYSQL_PASSWD/" /etc/proxysql/proxysql.cnf
    sed -i "s/adminWebMON/$WEBMON_PASSWD/" /etc/proxysql/proxysql.cnf
    sed -i "s/pr0xysql/$PSQLDBS_USER/" /etc/proxysql/proxysql.cnf
    sed -i "s/passw0rd/$PSQLDBS_PASSWD/" /etc/proxysql/proxysql.cnf
